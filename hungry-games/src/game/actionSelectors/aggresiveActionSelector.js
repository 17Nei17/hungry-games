...
function aggresiveActionSelector(actionType, user, diedUser, diedUserIndex) {
    switch (actionType) {
        ...
        case 'fire':
            return { text: aggresiveFireAction(getRandonNumber(aggresiveFireAction.caseLength), user.name, diedUser), isAlive: true, anotherUserIndex: diedUserIndex, isAggresiveAction: true, murdersNumber: user.murdersNumber + 1 };
        default:
